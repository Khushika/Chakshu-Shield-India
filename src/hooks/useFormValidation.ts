...
const useFormValidation = (schema: ValidationSchema): UseFormValidationReturn => {
...
export const enhancedValidations = {
  ...
  fraudType: {
    required: true,
    custom: (value: string) => {
      const validTypes = ['phishing', 'sms_fraud', 'call_fraud', 'email_spam', 'investment_scam', 'lottery_scam', 'tech_support_scam', 'romance_scam', 'other'];
      if (!validTypes.includes(value)) return 'Please select a valid fraud type';
      return null;
    },
  },
};
